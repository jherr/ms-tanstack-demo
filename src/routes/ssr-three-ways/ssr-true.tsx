...
export const Route = createFileRoute('/ssr-three-ways/ssr-true')({
  ssr: true,
  loader: async () => {
    try {
  ...
  component: SsrTruePage,
})

function SsrTruePage() {
  const data = Route.useLoaderData()

  return (
    <div className="page-wrap px-4 pb-8 pt-14">
      <div className="island-shell rise-in max-w-2xl mx-auto rounded-[2rem] px-6 py-10 sm:px-10 sm:py-14">
        <p className="island-kicker mb-1 text-emerald-600">ssr: true</p>
        <h1 className="display-title mb-2 text-3xl font-bold tracking-tight text-[var(--sea-ink)]">
          Full SSR
        </h1>
        <p className="mb-6 text-sm text-[var(--sea-ink-soft)]">
          Loader runs on the server on initial request, so fetch to the
          api-server (same machine, no CORS) succeeds.
        </p>
